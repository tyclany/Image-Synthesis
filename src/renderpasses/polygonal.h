...
struct PolygonalPass : RenderPass {
    ...
    bool init(const Config& config) override {
        ...
        for(int i = 0; i<nbVertices ; i++){
            v3f posV1 = scene.getObjectVertexPosition(emitter.shapeID,i);
            emitterVertexData.push_back(posV1.x);
            emitterVertexData.push_back(posV1.y);
            emitterVertexData.push_back(posV1.z);
        }
    ...
    void addShadows() {
        ...
        const float aspectRatio = (float)camera.window_width / (float)camera.window_height;
        const float scaling = tan(camera.field_of_view / 2.f);
        for(int i = 0; i< camera.window_height;i++){
            for(int j = 0; j<camera.window_width;j++) {
                double px = 2 * (j + 0.5) / camera.window_width - 1;
                double py = -(2 * (i + 0.5) / camera.window_height - 1);
                v4f windowPosition = v4f(px * aspectRatio * scaling, py * scaling, -1, 0);

                glm::mat4 inverseView = glm::inverse(glm::lookAt(camera.camera_position, camera.camera_look_at, camera.camera_up));
                glm::vec4 dir = inverseView * windowPosition;
                Ray ray = Ray(camera.camera_position, normalize(dir));

                v3f value = polygonalIntegrator->estimateVisDiffRealTime(ray, *sampler, emitter);
                int index = (camera.window_height - i - 1) * camera.window_width + j;
                cvTermData[3 * index] = (cvTermData[3 * index]*samplesAccumulated - value.x)/(samplesAccumulated+1);
                cvTermData[3 * index + 1] = (cvTermData[3 * index+1]*samplesAccumulated - value.y)/(samplesAccumulated+1);
                cvTermData[3 * index + 2] = (cvTermData[3 * index+2]*samplesAccumulated - value.z)/(samplesAccumulated+1);
            }
        }
        samplesAccumulated++;
        glBindTexture(GL_TEXTURE_2D, cvTermTexture);
    ...
    void render() override {
        ...
        for (auto& object : objects) {
            ...
            */
            ...
            glUniform1fv(emitterVerticesUniform, nbTriangles * 9, emitterVertexData.data());
            ...
            glUniform2f(windowSizeUniform, camera.window_width, camera.window_height);


            if(object.shaderIdx==DIFFUSE_SHADER_IDX) {
                object.albedoUniform = GLuint(glGetUniformLocation(object.shaderID, "albedo"));
